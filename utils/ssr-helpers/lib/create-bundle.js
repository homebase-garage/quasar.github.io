...
function createRequire(basedir, files, evaluateModule) {
  ...
  function _resolve(id) {
    return (
      resolveFromFiles(id) || nativeRequire.resolve(id, { paths: [basedir] })
    )
  }

  _resolve.paths = nativeRequire.resolve.paths.bind(nativeRequire.resolve)

  const _require = function _require(id) {
...
function createModule(options) {
  return {
    ...
    loaded: options.loaded !== void 0 ? options.loaded : false,
...
function getRewriteErrorTrace(rawMaps) {
  ...
  return async function rewriteTraceLineFn(err) {
