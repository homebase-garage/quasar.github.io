...
cd ../exports/material-symbols-rounded || exit 1

FILE="material-symbols-rounded.css"
...
for AGENT in "$AGENT_WOFF2" "$AGENT_WOFF"; do
  # download css
  URL=$(wget https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200 -O - --header="User-Agent: ${AGENT}" | \
    sed "s/local('.*'), //" | tr '()' \\n | grep https\*:// | head -n 1)
...
if [ -n "$VERSION" ]; then
  echo "QEXTRA_VERSION::material-symbols-rounded::${VERSION}"
else
  echo "Failed to determine version for material-symbols-rounded!!!"
  exit 1
fi
