...
cd ../exports/material-icons-outlined || exit 1

FILE="material-icons-outlined.css"
...
for AGENT in "$AGENT_WOFF2" "$AGENT_WOFF"; do
  # download css
  URL=$(wget https://fonts.googleapis.com/icon?family=Material+Icons+Outlined -O - --header="User-Agent: ${AGENT}" | \
    sed "s/local('.*'), //" | tr '()' \\n | grep https\*:// | head -n 1)
...
if [ -n "$VERSION" ]; then
  echo "QEXTRA_VERSION::material-icons-outlined::${VERSION}"
else
  echo "Failed to determine version for material-icons-outlined!!!"
  exit 1
fi
