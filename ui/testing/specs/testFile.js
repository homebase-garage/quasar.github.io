...
function createIgnoreComment(ignoreCommentIds) {
  ...
  while (index < ignoreCommentIds.length) {
    let line = ' *'
    while (
      line.length <= ignoreCommentLineMaxLen &&
      index < ignoreCommentIds.length
    ) {
...
function getTestFileMisconfiguration({ ctx, generator, json, testFile, opts }) {
  const errors = []
  const warnings = []
  ...
  if (Object.keys(testTree)[0] !== testTreeRootId) {
    errors.push(`Should contain a root describe('${testTreeRootId}')`)
  ...
  Object.keys(tree.children).forEach(categoryId => {
    ...
    if (categoryTree === null) {
      errors.push(`Found empty describe('${categoryId}'). Delete it.`)
      return
    }
    ...
    Object.keys(categoryTree).forEach(testId => {
      const { type: entryType } = categoryTree[testId]

      if (entryType !== 'describe') {
        errors.push(
          `Found a ${entryType}('${testId}') but it should be a describe()`
        )
        return
      }
      ...
      if (categoryTree[testId].children === null) {
        errors.push(`Found empty describe('${testId}')`)
      }
...
function getTestFileMissingTests({ ctx, generator, json, testFile }) {
  if (testFile.content === null || json === void 0) {
    return null
  }
  ...
  Object.keys(identifiers).forEach(jsonKey => {
    ...
    if (getTestId === void 0) {
      ...
      if (
        testFile.testTree[ctx.testTreeRootId].children[categoryId] !== void 0
      ) {
        return
      }
    ...
    Object.keys(categoryJson).forEach(entryName => {
      ...
      if (
        testFile.testTree[ctx.testTreeRootId].children[categoryId]?.children[
          testId
        ] !== void 0
      ) {
        return
...
export function getTestFile(ctx) {
  ...
  const testFile = {
    ...
    getMisconfiguration(opts) {
      if (generator === null) init()
      return getTestFileMisconfiguration({
        ctx,
        generator,
        json,
        testFile: this,
