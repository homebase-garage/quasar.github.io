...
const typeMap = {
  ...
  Date: {
    ...
    createExpectCall: ({ ref }) =>
      `expect(${ref}).$any([ expect.any(Date), expect.any(String), expect.any(Number) ])`,
    expectMatcher:
      'expect.$any([ expect.any(Date), expect.any(String), expect.any(Number) ])'
  },
...
function joinObject({ keyList, getValue, indent }) {
  const codeLines = []
  const commentLines = []
  ...
  const separator = `,\n${indent}  `
  const listSeparator =
    codeLines.length !== 0 && commentLines.length !== 0 ? `\n${indent}  ` : ''
...
function joinList({ keyList, getValue, indent, prefix = '[', suffix = ']' }) {
  const innerIndent = keyList.length === 1 ? indent : indent + '  '
...
export function getComponentPropAssignment({
  ...
  return `await wrapper.setProps(${props})` + `\n${indent}await flushPromises()`
}
...
export function getComponentMount({
  ...
  const target = {}
  const innerIndent = `${indent}    `
  const props = Object.keys(json.props || []).filter(
    propName => prop === propName || json.props[propName].required === true
  )
  ...
  if (slot !== null) {
    ...
    const nameAsObjKey = name.includes('-')
      ? /* example: 'navigation-icon' */ `'${name}'`
      : name
...
function getExpectMatcher({ jsonEntry, indent }) {
  ...
  if (jsonEntry.type.includes('Any') || jsonEntry.type.includes('FileList')) {
    return 'expect.anything()'
  }

  const list = joinList({
    keyList: jsonEntry.type,
    getValue: (type, innerIndent) =>
      getExpectMatcher({
...
export function getTypeTest({ jsonEntry, ref, indent }) {
  if (jsonEntry.values !== void 0) {
    const list = joinList({
      keyList: jsonEntry.values,
      getValue: key => key,
