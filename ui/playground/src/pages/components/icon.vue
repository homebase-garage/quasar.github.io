<template>
  <div class="q-layout-padding">
    ...
    >
      <div
        v-for="test in testSvgReTexts"
        :key="test.text"
        class="q-px-md row no-wrap items-center justify-between"
      >
        <div>{{ test.text }}</div>
    ...
    <div class="q-mt-xl">
      <div v-for="set in sets" :key="set.setName">
        ...
        <div>
          <q-chip
            class="q-ma-xs"
            v-for="theIcon in set.icons"
            :key="set.setName + theIcon.name + theIcon.val"
          >
    ...
    <div class="q-mt-xl">
      ...
      <q-input
        v-model="text"
        clearable
        clear-icon="img:https://cdn.quasar.dev/logo-v2/svg/logo.svg"
      />
    </div>
  </div>
</template>
...
const iconOptions = [
  ...
  { value: 'mdi-airballoon', label: 'A MDI v7 icon' },
  { value: mdiAirballoon, label: 'A MDI v7 SVG icon' },
