  ...
  describe('[Props]', () => {
    describe('[(prop)size]', () => {
      test('type String has effect', async () => {
        ...
        expect(target.$style('font-size')).toBe('')

        await wrapper.setProps({ size: '100px' })
        await flushPromises()

        expect(target.$style('font-size')).toContain('100px')

        await wrapper.setProps({ size: 'sm' })
        await flushPromises()

        expect(target.$style('font-size')).toBe(`${useSizeDefaults.sm}px`)
      })
    })
    ...
    describe('[(prop)font-size]', () => {
      test('type String has effect', async () => {
        ...
        expect(wrapper.get('.q-avatar').$style('font-size')).not.toBe(size)

        expect(wrapper.get('.q-avatar__content').$style('font-size')).not.toBe(
          fontSize
        )
        ...
        expect(wrapper.get('.q-avatar').$style('font-size')).toBe(size)

        expect(wrapper.get('.q-avatar__content').$style('font-size')).toBe(
          fontSize
        )
      })
    })
    ...
    describe('[(prop)icon]', () => {
      test('type String has effect', async () => {
        ...
        expect(wrapper.get('.q-avatar').find('.q-icon').exists()).toBe(false)

        await wrapper.setProps({ icon: propVal })
        await flushPromises()

        expect(wrapper.get('.q-avatar').get('.q-icon').text()).toContain(
          `${propVal}`
        )
      })
    })
    ...
    describe('[(prop)square]', () => {
      test('type Boolean has effect', async () => {
        ...
        expect(target.classes()).not.toContain('q-avatar--square')
