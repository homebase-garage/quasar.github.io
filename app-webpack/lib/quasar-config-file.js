...
const {
  quasarEsbuildInjectReplacementsDefine,
  quasarEsbuildInjectReplacementsPlugin
} = require('./plugins/esbuild.inject-replacements.js')
...
module.exports.QuasarConfigFile = class QuasarConfigFile {
  ...
  constructor({ ctx, host, port, verifyAddress, watch }) {
    ...
    const quasarConfigFileExtension =
      appPaths.quasarConfigOutputFormat === 'esm'
        ? 'mjs'
        : appPaths.quasarConfigOutputFormat
  ...
  #createEsbuildConfig() {
    ...
    return {
      ...
      resolveExtensions: [
        appPaths.quasarConfigOutputFormat === 'esm' ? '.mjs' : '.cjs',
        '.js',
        '.mts',
        '.ts',
        '.json'
  ...
  async #computeConfig(quasarConfigFn, failOnError) {
    ...
    if (cfg.build.webpackTranspile === true) {
      cfg.build.webpackTranspileDependencies =
        cfg.build.webpackTranspileDependencies.filter(uniqueRegexFilter)
    ...
    if (!cfg.build.esbuildTarget.browser) {
      cfg.build.esbuildTarget.browser = [
        'es2022',
        'firefox115',
        'chrome115',
        'safari14'
    ...
    cfg.build.publicPath = this.#ctx.mode.bex
      ? '/www'
      : cfg.build.publicPath &&
          ['spa', 'pwa', 'ssr'].includes(this.#ctx.modeName)
    ...
    if (this.#ctx.dev) {
          ...
          setupMiddlewares: (middlewares, opts) => {
            ...
            if (!this.#ctx.mode.ssr) {
              ...
              if (cfg.build.ignorePublicFolder !== true) {
                app.use(
                  cfg.build.publicPath || '/',
                  express.static(appPaths.resolve.app('public'), {
                    maxAge: 0
                  })
                )
              }
              ...
              if (this.#ctx.mode.cordova) {
                const folder = appPaths.resolve.cordova(
                  `platforms/${this.#ctx.targetName}/platform_www`
                )
      ...
      if (
        this.#ctx.vueDevtools === true ||
        cfg.devServer.vueDevtools === true
      ) {
      ...
      if (this.#ctx.mode.bex === true && this.#ctx.target.firefox === true) {
          ...
          ;['ca', 'pfx', 'key', 'cert'].forEach(prop => {
            if (typeof options[prop] === 'string') {
              try {
                ...
                delete options[prop]
                warn(
                  `The devServer.server.options.${prop} file could not be read. Removed the config.`
                )
              }
            }
          })
    ...
    if (cfg.build.gzip) {
      const gzip = cfg.build.gzip === true ? {} : cfg.build.gzip
      let ext = ['js', 'css']
    ...
    if (this.#ctx.dev) {
      const getUrl = hostname =>
        `http${cfg.devServer.server.type === 'https' ? 's' : ''}://${hostname}:${cfg.devServer.port}${cfg.build.publicPath}`
      const hostname =
